...
int load_and_build_data(const char *filename, int **ptr_arr_data)
{
    ...
    if (ptr_arr_data == NULL || *ptr_arr_data != NULL) {
    ...
    *ptr_arr_data = (int *) calloc(num_datapoints, sizeof(int));
...
int free_loaded_data(int *ptr_arr_data)
{
    if (ptr_arr_data == NULL) {
        printf("warning, passing already null pointer\n");
        return 0;
    }
